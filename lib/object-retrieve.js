...
// The query is unchanging, so there is no reason to assemble the query
// each time a retrieval is performed.
const query = ' \
    (SELECT \
    ...
    build_object(obj_id, expand, cb) {
        ...
        query_data(obj_id, function(data, err) {
            if (err) {
                ...
                // Take the data from the first key of the collapse data
                let drs_obj_data = data[_.keys(data)[0]];

                build_drs_object(obj_id, expand, drs_obj_data,
                    function(object, build_err) {
                        if (build_err) {
                            cb(null, build_err);
                        } else {
                            cb(object, null);
...
// The ObjectRetrive class is implemented as a singleton, so that only
// one instance of a the implementation can be instantiated and used.
// This is done with a static get_instance() method that instantiates
// another, private, class: PrivateObjectRetrieve, which is otherwise
// inaccessible.
class ObjectRetrieve {
    ...
    // Provides the caller with the only method of retrieving an
    // instance of the PrivateObjectRetrieve class.
...
function format_contents(data) {
    let contents = {
        drs_uri: [ 'drs://{}/{}'.format(drs_domain, data['drs_id']) ],
...
// Retrieve the data for the given DRS object ID from the database.
//
// obj_id : The DRS object id to retrieve.
// cb     : A (results, err) callback that is invoked with the results.
//
...
// A convenience method to re-establish the connection to the database
// in case the connection is dropped or gets into an unusable state.
function reconnect_to_db() {
    logger.debug('In reconnect_to_db.');
...
// Takes raw data in row format, and instantiates a DrsObject object
// and returns it.
//
// obj_id : The object ID that is being built
// expand : A boolean to indicate if the contents of a bundle should
//          be built recursively per the DRS API specification.
// data   : An object containing the information from the database
//          that is used to build the DrsObject.
// cb     : A (drs_object, error) callback.
...
// Establish the database connection. Accepts a callback as an argument.
// The callback is invoked with (success, error) arguments as is usual
// and customary, when the connection is established (or an error occurs)
function start_connection(cb) {
    ...
    let obscure = true;
    let mysql_password = config.value('global', 'mysql_pass', obscure);
