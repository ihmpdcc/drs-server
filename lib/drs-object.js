...
module.exports = class DrsObject {
    static #drs_domain = null;

    static set domain(domain) {
        DrsObject.#drs_domain = domain;
    }

    static get domain() {
        return DrsObject.#drs_domain;
    }

    constructor(id, drs_id, size) {
        if (_.isNil(DrsObject.#drs_domain)) {
            throw new Error("Set domain first with set_domain().")
        }

        this.id = id;
        this.access_id_base = drs_utils.generate_access_id(drs_id);
        this.drs_id = drs_id;
        this._size = size;
        this.is_blob = true;
    }
    ...
    add_access_method(access_method) {
        ...
        // Set the access id to the access_id_base with a numeric index suffix
        let access_index = this.access_methods.length + 1;
        let access_id = this.access_id_base + "-" + access_index;
        access_method.access_id = access_id;

        // Add the access method to our list of access methods.
    ...
    to_obj() {
        ...
        if (! _.isNil(this._sha256)) {
            obj['checksums'].push(
               {
                   'checksum': this._sha256,
                   'type': 'sha-256'
               }
            );
        }
